...
params:
  Solutions.directory:
    label: Pick directory with solutions (this should be fullpathto..HERE/Model ..
      Solution/.db)
    value: //plexossql/Data/bmcbenne/RTS-GMLC-geodecomp/RTS-GMLC/RTS_Data/FormattedData/PLEXOS/05_03_18
---
...
comp.table$category = factor(comp.table$category, levels = rev(names(color.code)))

p <- ggplot() + geom_point(data = comp.table,aes(x = var.cost,y = Fraction,color = category,size = Cap),alpha = 0.7) +
...
p
# ggsave('commit-plot.png',p,height = 2.75,width = 3.5,dpi = 600)
