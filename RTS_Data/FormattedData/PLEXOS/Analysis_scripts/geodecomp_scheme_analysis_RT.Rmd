...
```{r commit-plot-abb, include=FALSE,eval = TRUE, fig.width=3.5, fig.height=3}
...
ggsave("commit-plot-print.png",p,height = 5,width = 3.5)
...
p <- ggplot() + geom_line(data = flows.table,
      ...
      scale_x_datetime(labels = date_format("%b"),breaks=date_breaks('4 months'), expand = c(0, 0)) +
      plot_theme + 
...
p
ggsave('interface-timeseries-print.png',p,height = 5,width = 3.5)
...
KSI_calcluation = function(KSI.step = 0.01,line.filter = c(),category.filter = c("Interregion_AC","Interregion_DC")){
  
  series.length = length(unique(flow.table[,time]))
  
  # relies on not having different line limits between scenarios
  flow.range = unique(line.rt.limits[,.(name,category,`Export Limit`,`Import Limit`)])
  
  flow.table = copy(interval.rt.flow)
  
  if(length(line.filter)>0){
    flow.range = flow.range[name %in% line.filter]
    flow.table = flow.table[name %in% line.filter]
  }
  if(length(category.filter)>0){
    flow.range = flow.range[category %in% category.filter]
    flow.table = flow.table[category %in% category.filter]
  }
  flow.range[,category:=NULL]
...
KSI.cacluation = KSI_calcluation(line.filter = c(),category.filter = c("Interregion_AC","Interregion_DC"))
...
ggsave('KSI-plot-print.png',p,height = 5,width = 3.5)

```



```{r KSI-combo, include=FALSE,eval = TRUE, fig.width=6, fig.height=3}

KSI.cacluation = KSI_calcluation(line.filter = c("AB3"),category.filter = c("Interregion_AC","Interregion_DC"))
KSI.table = KSI.cacluation[["KSI.table"]]
flow.range = KSI.cacluation[["flow.range"]]
...
flows.table = copy(interval.rt.flow[name == "AB3" &
                                      category %in% c("Interregion_AC","Interregion_DC")])

p1 <- ggplot() + geom_point(data = KSI.plot.table,aes(x = value,y = p,color = scenario),size = 0.15) + 
    geom_vline(data = flow.range, aes(xintercept = `Import Limit`),size = 0.15,color = 'black') + 
    geom_vline(data = flow.range, aes(xintercept = `Export Limit`),size = 0.15,color = 'black') + 
    scale_color_manual(values = scenario.colors) +
    facet_grid(name ~ .,scales = 'fixed') + 
...
p2 <- ggplot() + geom_line(data = flows.table,
      aes(x = time,y = value, color = scenario),size = 0.4,alpha = 0.5) + 
      scale_color_manual(values = scenario.colors) + 
      scale_x_datetime(labels = date_format("%b"),breaks=date_breaks('4 months'), expand = c(0, 0)) +
      plot_theme + 
...
p <- plot_grid(p2,p1,labels = c("", ""), ncol = 1,align = "v",
                        rel_heights = c(1,1.2))
...
```{r KSI-combo-print, include=FALSE,eval = TRUE, fig.width=6, fig.height=3}

print(p)

ggsave("p1.png",p1,height = 3.5,width = 3.5)
ggsave("p2.png",p2,height = 3.5,width = 3.5)

ggsave('KSI-combo-print.png',p,height = 4.5,width = 3.5)
