...
SINGLE_OPT_EDGE_price_flow_plots = function(){
    ...
    # get border regions for price calculation --------------------------------
    border.regions = int.region.NI[,lapply(.SD,function(x) sum(x)/1000),
                                   by = c('ISO.From','ISO.To','Region.From','Region.To'),
                                   .SDcols = c('value')]
    
    border.regions = border.regions[!(ISO.From == ISO.To) & !(value == 0)]
    
    
    border.regions[,Interface:=paste0(ISO.From," - ",ISO.To)]
    border.regions = merge(border.regions,SEAMS.interfaces,by = c('ISO.From','ISO.To','Interface'))
    
    border.regions[,c('value','color'):=NULL]
    # -------------------------------------------------------------------------
    ...
    time.key = data.table(time = sort(unique(int.region.NI[,time])),
                          Period = seq(length(unique(int.region.NI[,time]))))
    ...
    int.region.from.price = merge(int.node.price[,.(Region.From = category,Node.From = name,time,Price.From = value)],
                                  border.regions,
                                  by = c('Region.From'))
    
    int.region.from.price = int.region.from.price[,lapply(.SD,mean),
                                                  by = c('Interface','time'),
                                                  .SDcols = c('Price.From')]
    
    int.region.to.price = merge(int.node.price[,.(Region.To = category,Node.To = name,time,Price.To = value)],
                                border.regions,
                                by = c('Region.To'))
    
    int.region.to.price = int.region.to.price[,lapply(.SD,mean),
                                              by = c('Interface','time'),
                                              .SDcols = c('Price.To')]
    ...
    int.region.price = int.region.price[,.SD,.SDcols = names(int.region.NI)]
