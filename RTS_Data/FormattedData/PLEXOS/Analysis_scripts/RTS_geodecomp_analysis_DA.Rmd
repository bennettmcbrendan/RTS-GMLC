...
commit.changes[,fraction:=paste0(as.character(round(100*(`Committed only by multi-operator` + 
                                            `Committed only by single-operator`)/
                                              (`Committed` + 
                                               `Committed only by single-operator` +
                                               `Committed only by multi-operator`)),1),'%')]

commit.changes[,legend:='Fraction of \ncommitments \nwhich vary']
...
p <- ggplot() + geom_bar(data = int.gen,aes(x = category,y = value,fill = variable),
                         stat = 'identity',color = 'black') + 
    # coord_polar("y",start = 0) + 
    # facet_wrap(~category,scales = 'fixed') +
    geom_label(data = commit.changes,aes(x = category,y = y+500,label = fraction,color = legend),
               size = 3,fill = 'black',show.legend = FALSE) +
    scale_color_manual(values = c('Fraction of \ncommitments \nwhich vary' = 'white')) +
...
ggsave("commit_barplot.png",p,height = 4,width = 3.5)
